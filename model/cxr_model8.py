...
class CxrModel8(pl.LightningModule):
    def __init__(
        self, lr, classes, loss_init_args, timm_init_args,
        lambda_pa=0.3, lambda_lat=0.3, lambda_consistency=0.5, htb_alpha=2.0
    ):
        super(CxrModel8, self).__init__()
        ...
        self.htb_alpha = htb_alpha # Kappa 민감도
    ...
    def shared_step(self, batch, batch_idx, mode='train'):
        ...
        if has_pa.any() or has_lat.any():
            ...
            if has_pa.any():
                loss_cons += kappa_pa * F.mse_loss(prob_fusion[has_pa], prob_pa[has_pa])
            if has_lat.any():
                loss_cons += kappa_lat * F.mse_loss(prob_fusion[has_lat], prob_lat[has_lat])
        ...
        return dict(loss=total_loss, pred=torch.sigmoid(logits_fusion).detach(), label=label,
                    ...
                    kappa_pa=kappa_pa, kappa_lat=kappa_lat) # 로깅용 리턴 추가
