...
from model.ce import BCELosswithDiffLogits, cross_entropy, binary_cross_entropy, partial_cross_entropy, kpos_cross_entropy
...
def get_loss(type, class_instance_nums, total_instance_num):
    if type == 'bce':
        ...
        return ResampleLoss2(class_instance_nums, total_instance_num)
    else:
...
class ResampleLoss2(nn.Module):
    ...
    def forward(self,
        ...
        if self.focal:
            logpt = - self.cls_criterion(
                cls_score.clone(), label, weight=None, reduction='none',
                avg_factor=avg_factor)
            _assert_finite(logpt, "resample2.logpt@before_pt")
            # pt is sigmoid(logit) for pos or sigmoid(-logit) for neg
            pt = torch.exp(logpt)  
            _assert_finite(logpt, "resample2.logpt@before_pt")     
            loss = self.cls_criterion(
            ...
            _assert_finite(logpt, "resample2.logpt@before_pt")     
            loss = ((1 - pt) ** self.gamma) * loss
            _assert_finite(loss, "resample2.loss@after_focal")
            loss = self.balance_param * loss
        else:
            loss = self.cls_criterion(cls_score, label.float(), weight,
                                      reduction=reduction)     
    ...
    def lpl_imbalance(self, logits, labels, prop, nonzero_var_tensor, zero_var_tensor, normalized_sigma_cj, normalized_ro_cj, normalized_tao_cj):
        ...
        coef_cj = coef_cj * (torch.ones(C, C).cuda()- torch.eye(C).cuda()) + torch.diag(coef_cc)
        ...
        _assert_finite(head_coef, "lpl.head_coef")
        _assert_finite(tail_coef, "lpl.tail_coef")
    ...
    def logit_reg_functions(self, labels, logits, norm_prop,
        ...
        if 'init_bias' in self.logit_reg:
            ...
            logits = logits.view(B, C, 1).expand(B, C, C).clone()
            labels = labels.view(B, C, 1).expand(B, C, C).clone()
            ...
            perturb = self.lpl_imbalance(
                logits, labels, norm_prop, nonzero_var_tensor,
                zero_var_tensor, normalized_sigma_cj, normalized_ro_cj,
                normalized_tao_cj
            )
            _assert_finite(perturb, "logit_reg.perturb@from_lpl")

            logits = logits + perturb
            _assert_finite(logits, "logit_reg.logits@after_lpl")
        ...
        if 'neg_scale' in self.logit_reg:
            ...
            if weight is not None:
                weight = weight.view(weight.size(0), weight.size(1), 1).expand(
                    weight.size(0), weight.size(1), weight.size(1)
                )
                weight = weight / self.neg_scale * (1 - labels) + weight * labels
                _assert_finite(weight, "logit_reg.weight@after_neg_scale")
